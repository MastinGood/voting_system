...
class Voters
{
    public function INSERT_VOTER($name, $course, $year, $stud_id,$conformation_code) {
        global $db;
        ...
        if($result->num_rows > 0) {
            ...
            //Insert voter
            $sql = "INSERT INTO voters(name, course, year, stud_id,conformation_code)VALUES(?, ?, ?, ?, ?)";
            if(!$stmt = $db->prepare($sql)) {
                echo $stmt->error;
            } else {
                $stmt->bind_param("sssss", $name, $course, $year, $stud_id, $conformation_code);
            }
            if($stmt->execute()) {
